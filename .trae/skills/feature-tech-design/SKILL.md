...
1. **需求文档就绪**：`specs/features/{版本号}/{功能名称}.md` 存在且包含完整的验收标准。如果缺失或不完整，提示用户先完成需求澄清，不要在需求模糊的基础上做技术设计。

2. **项目认知建立**：
   - 读取 `specs/{版本号}/需求概述.md` 是否存在，存在则按照该文档的内容进行操作(必须)
...
2. 填充内容，生成最终文档
3. 保存到 `specs/features/{版本号}/{功能名称}_技术方案.md`
