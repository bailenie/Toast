...
## 你的任务
基于技术栈 (`specs/技术栈.md`) 和核心业务 (`specs/{版本号}/需求概述.md`)，生成具体的项目目录结构。
...
**当前阶段**: 架构与设计阶段 (Architecture & Design)
 ...
 1.  **读取上下文**：
    ...
    *   读取 `specs/{版本号}/需求概述.md`：提取“核心板块”（如 Auth, User, Order），将它们映射到模块目录中。
...
3. 填好后保存为 `specs/features/项目结构.md`。

---

## 交互准则
- **适配性**：目录结构必须符合所选技术栈的最佳实践（例如：Next.js 14 使用 `app` router，Django 使用 `apps`）。
- **最终交付**：当文档内容被用户确认后，请将其保存到 `specs/features/项目结构.md`。
