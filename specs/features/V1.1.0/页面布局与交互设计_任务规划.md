...
- [ ] `npx prisma studio` 可以看到所有新表
- [ ] 现有数据的 `petFishGrowth` 正确继承了 `petFishExp` 的值
- [ ] User 模型不再有 `joinedCircleId` 和 `privateCircleId` 字段
- [ ] Circle 模型不再有 `isPrivate` 字段
...
- [ ] Decoration 表中有5条记录
- [ ] 每条记录的 name、icon、price、description 与设计一致
...
**验证标准**：
- [ ] `POST /api/circles` 创建鱼圈后 isActive=false，返回邀请码和 expiresAt
- [ ] `POST /api/circles/join` 通过邀请码加入成功，memberCount 正确递增
- [ ] 第2人加入后鱼圈 isActive 自动变为 true
- [ ] 过期邀请码加入返回 INVITE_EXPIRED 错误
- [ ] 已在鱼圈的用户再次加入返回 CIRCLE_ALREADY_MEMBER 错误
- [ ] `GET /api/circles` 返回用户加入的所有鱼圈列表
- [ ] 所有现有测试通过
...
**验证标准**：
- [ ] 签到成功后 SignRecord 表新增一条记录
- [ ] 签到成功后 coinBalance +1
- [ ] 同一天同一鱼圈重复签到返回 ALREADY_SIGNED 错误
- [ ] 非鱼圈成员签到返回 NOT_MEMBER 错误
- [ ] sign-status 正确返回本周签到天数数组
...
**验证标准**：
- [ ] `GET /api/decorations` 返回5款装饰
- [ ] 购买成功后 CircleDecoration 表新增记录，coinBalance 减少
- [ ] 鱼币不足返回 INSUFFICIENT_COINS 错误
- [ ] 重复购买返回 ALREADY_PURCHASED 错误
- [ ] 非成员购买返回 NOT_MEMBER 错误
...
**验证标准**：
- [ ] 摸鱼后 petFishGrowth +1（不是 +5）
- [ ] 成长值达到10时升级到 level 2
- [ ] 成长值达到30时升级到 level 3（需累计 10+20=30）
- [ ] 进化形态与等级对应正确
- [ ] API 接收 circleId 参数，操作对应鱼圈的数据
- [ ] 所有现有测试通过
...
**验证标准**：
- [ ] 登录后自动加载鱼圈列表
- [ ] 默认选中第一个鱼圈（或 localStorage 中保存的）
- [ ] setActiveCircle 切换后，其他组件能感知到变化
- [ ] 切换后 activeCircleId 保存到 localStorage
- [ ] 没有鱼圈时 activeCircleId 为 null
...
- [ ] 页面左侧有200px宽的鱼圈栏
- [ ] 主内容区占据剩余空间
- [ ] 顶部 Navbar 显示当前鱼圈名称和用户头像
- [ ] 底部有 [💬 蛐蛐间] [🎮 摸鱼鱼] Tab 栏
- [ ] 默认显示聊天室（蛐蛐间）
- [ ] 切换鱼圈后主内容区刷新
- [ ] 底部 Tab 切换正常
- [ ] 进入窝囊费页面时，左侧鱼圈栏隐藏，底部Tab栏隐藏 ✅ 2026-06-22
- [ ] 窝囊费页面时，导航栏显示返回按钮（← 返回） ✅ 2026-06-22
- [ ] 点击返回按钮跳转到 /home，左侧栏和底部Tab恢复显示 ✅ 2026-06-22
...
**验证标准**：
- [ ] 无鱼圈的用户登录后看到引导页
- [ ] 点击"加入同事的鱼圈"打开 JoinCircleModal
- [ ] 点击"创建新鱼圈"打开 CreateCircleModal
- [ ] 点击"暂时跳过"关闭引导页
- [ ] 加入/创建成功后引导页消失，进入鱼圈
...
- [ ] Prisma 迁移脚本在测试环境验证通过（数据不丢失）
- [ ] 左侧栏切换鱼圈 < 500ms 响应
- [ ] 创建鱼圈邀请码1小时过期逻辑通过时间模拟测试
- [ ] 所有现有测试（auth、salary、chat）不受影响，全部通过
- [ ] UI 遵循 design.md 规范：cute-shadow、border-ink、font-display、rounded 元素
